...
2. Basics of Pelican and Github Pages
    - Getting Set Up
...
6. Modifying the Style
7. Modifying the Theme
8. Modifying the Sidebar Links
9. Tips and Tricks
...
* Working with a fork of a Git repository you found on GitHub.
...
Next, you must fork the push-redux GitHub repo. Once the repo is forked, clone the repo so you can edit your fork locally.
...
The `themes/` folder contains folders of themes that determine the layout and style of the Push Redux website. Pelican uses the Jinja template-ing engine to create themes. Modifying the contents of the `themes/` folder will only be necessary if you intend to modify the sites layout and style. 
...
If you are on a Unix machine, this is make easier by the `Makefile`. Simply run `make html`.

>Be sure to check the console for any errors that occured wile building the site. Ignore the following warning: "WARNING: empty slug for path/to/push-redux/content/pages/index.md. You can fix this by adding a title or a slug to your content".

Now open the `path/to/push-redux/docs/index.html` file in your web browser to preview your changes.

>BE WARNED: As you navigate between pages, you may find yourself having to click on the `index.html` file for the folder you are in. This is normal. GitHub Pages displays the `index.html` file in the current directory by default, but your browser won't. Once your changes go live on the main repository, this will not happen anymore.
...
Once your changes are have been pushed to your GitHub fork, open a pull request between your fork and the main `push-redux` repository. This can be done from the GitHub website. More information about pull requests can be found here.
...
Pelican uses the Jinja theme engine to manage the theme of the website. If you look in the `themes/push-redux-theme` folder you should see two folders: `static/` and `templates/`. As mentioned in the previous section, the `static/` folder is where you put all of the external resources that will be included in the HTML of the theme. Javacript files, css style sheets, image files, etc should all be placed in the `static/` folder.
...
For larger changes to the `push-redux-theme` you will have to consult the Jinja documentation.

#### Modifying the Sidebar Links

Modifying the sidebar links is done by modifying values in the `pelicanconf.py` file. In the `pelicanconf.py` file, there is a variable called `LINKS` which is defined as nested tuples. Each tuple consists of 1) the text that should appear in the sidebar and 2) the URL which the link should point to.
